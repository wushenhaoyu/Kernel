...
def run_sampling_task(tasks: List[Path],
                      llm: LLM,
                      run_dir: Path,
                      *,
    ...
    for task in tasks:
        ...
        for b in range(samples_per_task):
            branch_dir = task_root / f"branch_{b:03d}"
            ...
            (branch_dir / "llm_io" / "reply.txt").write_text(output, encoding="utf-8")


            eval_file = branch_dir / "eval.json"
            summary   = {
                "branch": b,
                "compiled": False,
                "correct": False,
            ...
            if not ok:
                summary["error"] = "compile"; summary["error_log"] = build_log
            ...
            try:
                ...
                spec = importlib.util.spec_from_file_location(f"mod_b{b}", code_file)
                ...
                summary["error"] = "import"; summary["error_log"] = traceback.format_exc()
                eval_file.write_text(json.dumps(summary), encoding="utf-8")
                continue
            ...
            try:
                ...
                summary["error"] = "correctness"; summary["error_log"] = traceback.format_exc()
                eval_file.write_text(json.dumps(summary), encoding="utf-8")
                continue
            ...
            try:
                ...
                summary["error"] = "benchmark"; summary["error_log"] = traceback.format_exc()
                eval_file.write_text(json.dumps(summary), encoding="utf-8")
                continue

            eval_file.write_text(json.dumps(summary), encoding="utf-8")
            print(f"[{task_name}][b{b:03d}] speedup {summary['speedup']:.3f}x")

            branch_meta.append(summary)

        (task_root / "branch_meta.json").write_text(json.dumps(branch_meta), encoding="utf-8")